...
class BaseEvaluator(ABC):
    ...
    @abstractmethod
    def evaluate(
        self, current_data: Dict, previous_data: Optional[Dict], context: Any
    ) -> Optional[Any]:
    ...
    def evaluate(
        ...
        if current_odds < self.min_odds:
            return AlertResult(
                ...
                message=(
                    f"{context.horse_name} ({context.program_number}) odds "
                    f"dropped to {current_odds:.1f} at {context.track_name}"
                ),
        ...
        if current_odds > self.max_odds:
            return AlertResult(
                ...
                message=(
                    f"{context.horse_name} ({context.program_number}) odds "
                    f"rose to {current_odds:.1f} at {context.track_name}"
                ),
...
class RateOfChangeEvaluator(BaseEvaluator):
    ...
    def evaluate(
        ...
        change = abs((current_odds - previous_odds) / previous_odds) * 100
        ...
        if change >= self.min_change_percent:
            ...
            return AlertResult(
                ...
                message=(
                    f"{context.horse_name} odds {direction} by "
                    f"{change:.1f}% (from {previous_odds:.1f} to "
                    f"{current_odds:.1f})"
                ),
    ...
    def evaluate(
        ...
        if payout < self.min_payout:
            return AlertResult(
                ...
                message=(
                    f"Low exacta payout ${payout:.2f} for combination at "
        ...
        if payout > self.max_payout:
            return AlertResult(
                ...
                message=(
                    f"High exacta payout ${payout:.2f} for combination at "
                    f"{context.track_name}"
                ),
