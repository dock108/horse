...
class AlertEngine:
    ...
    def _init_evaluators(self) -> Dict[str, Any]:
        ...
        if self.config.enabled:
            evaluators["win_threshold"] = WinOddsThresholdEvaluator(
                min_odds=self.config.win_odds_min, max_odds=self.config.win_odds_max
            )
        ...
        for entry_data in snapshot_data.get("entries", []):
            ...
            for evaluator_name, evaluator in self.evaluators.items():
                ...
                result = evaluator.evaluate(
                    ...
                    previous_data=(
                        self._get_previous_entry_data(
                            previous_snapshot, entry_data.get("entry_id")
                        )
                ...
                if result and result.should_trigger:
                    ...
                    if not self.suppressor.is_suppressed(result):
                        ...
                        alerts.append(alert)
                        logger.info(
                            f"Alert triggered: {result.alert_type} - "
                            f"{result.message}"
                        )
    ...
    def _evaluate_exacta_alerts(self, snapshot_data: Dict) -> List[Alert]:
        ...
        for combo, payout in exacta_data.get("probables", {}).items():
            ...
            exacta_min = self.config.exacta_min_payout
            if exacta_min and payout < exacta_min:
                result = AlertResult(
                    should_trigger=True,
                    alert_type="exacta_low",
                    message=(
                        f"Low exacta payout ${payout:.2f} for {combo} at "
                        f"{context.track_name}"
                    ),
            ...
            exacta_max = self.config.exacta_max_payout
            if exacta_max and payout > exacta_max:
                result = AlertResult(
                    should_trigger=True,
                    alert_type="exacta_high",
                    message=(
                        f"High exacta payout ${payout:.2f} for {combo} at "
                        f"{context.track_name}"
                    ),
    ...
    def _save_alerts(self, alerts: List[Alert]):
        ...
        for alert in alerts:
            ...
            """
            alert.alert_id = self.db.execute_insert(
                query,
                (
                    alert.triggered_at,
                    alert.race_id,
