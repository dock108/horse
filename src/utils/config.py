...
class ConfigManager:
    ...
    def load_config(self, config_path: str = "config/config.yaml"):
        ...
        if not config_file.exists():
            ...
            if example_file.exists():
                logger.warning(
                    f"Config file not found, using example: " f"{example_file}"
                )
    ...
    def _validate_config(self):
        ...
        if self._config.get("email", {}).get("enabled"):
            ...
            for email_field in required_email:
                if not self._config["email"].get(email_field):
                    logger.warning(
                        f"Email {email_field} is required when email is " f"enabled"
                    )
    ...
    def get_alert_config(self) -> AlertConfig:
        ...
        return AlertConfig(
            ...
            max_alerts_per_race=alerts.get("suppression", {}).get(
